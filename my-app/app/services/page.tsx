  ...
  return (
    <main className="bg-white">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service) => (
              <div
                key={service.id}
                className="bg-brand-light rounded-lg p-8 border border-slate-200 hover:border-brand-accent hover:shadow-lg transition-all duration-300"
              >
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {service.industries.map((industry) => (
                      <span
                        key={industry}
                        className="bg-white text-brand-dark text-xs px-3 py-1 rounded-full border border-slate-200"
                      >
                        {industry}
                      </span>
                    ))}
                ...
                <Link href={`/work/${service.caseStudies[0]}`}>
                  <div className="text-brand-accent font-semibold group hover:translate-x-2 transition-transform duration-300 cursor-pointer flex items-center gap-2">
                    View Case Study
                    <ArrowRight size={16} />
                  </div>
                </Link>
      ...
      {services.map((service, idx) => (
        <section
          key={service.id}
          className={`py-20 px-6 md:px-12 ${idx % 2 === 0 ? 'bg-white' : 'bg-brand-light'}`}
        >
          <div className="max-w-7xl mx-auto">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className={idx % 2 === 1 ? 'md:order-2' : ''}>
                <h2 className="text-4xl font-bold text-brand-dark mb-6 font-serif">
                  {service.title}
                </h2>
                ...
                <h3 className="text-xl font-bold text-brand-dark mb-4">Benefits</h3>
              ...
              <div className={idx % 2 === 1 ? 'md:order-1' : ''}>
                <div className="bg-white rounded-lg p-8 border border-slate-200 shadow-lg">
                  <h3 className="text-xl font-bold text-brand-dark mb-6 font-serif">
                    Our Approach
                  </h3>
                  <div className="space-y-4 mb-8">
                    {service.approach.map((item, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <div className="w-8 h-8 bg-brand-accent rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                          {i + 1}
                        </div>
                  ...
                  <h3 className="text-xl font-bold text-brand-dark mb-4 font-serif">
                    Deliverables
                  </h3>
